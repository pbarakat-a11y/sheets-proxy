...
const MONTHS = {
  ...
  may: {
    ...
    reps: [
      { name: "Ahjei Holieway",   quota: 8900, closed: 3564, low: 3424, high: 5094, forecast: 4259 },
      { name: "Brandon Hardy",    quota: 8900, closed: 1919, low: 1584, high: 1884, forecast: 1734 },
      { name: "Aram Antranigian", quota: 6800, closed: 1910, low: 370,  high: 1995, forecast: 1183 },
      { name: "Graydon Brown",    quota: 6800, closed: 3228, low: 3228, high: 3703, forecast: 3466 },
      { name: "Chris Sims",       quota: 6800, closed: 4618, low: 3297, high: 7247, forecast: 5272 },
      { name: "Henry Moffly",     quota: 0,    closed: 0,    low: 0,    high: 0,    forecast: 0    },
    ],
  },
};
...
// ── Month view ────────────────────────────────────────────────────────────────
function MonthView({ monthKey }) {
  const raw = MONTHS[monthKey];
  if (!raw) return (
...
export default function Dashboard() {
  ...
  useEffect(() => {
    const load = async () => {
      ...
      try {
        ...
        if (values) { setLiveData({ values }); setLiveError(null); }
  ...
  const mayData = enrichMonth(MONTHS.may);
  ...
  return (
    <div style={{ fontFamily: "'DM Mono', monospace", background: "#070d1a", minHeight: "100vh", color: "#e2e8f0", paddingBottom: 60 }}>
      ...
      <div style={{ padding: "24px 28px", maxWidth: 1100 }}>
        ...
        {["feb","mar","apr","may"].includes(activeTab) && <MonthView monthKey={activeTab} />}
      </div>
    </div>
  );
}
